...
*********************************************/	
	...
	function setMeeting (string _name,string _time, uint _stake)  {
		...
		address[] memory latePoeple;
		Meeting memory meeting = Meeting(_name, _time, _stake, 0, 0, meet, latePoeple);
	...
	function participate (uint _id, uint _userId) public  {

		require( hiveParticipants[participantList[_userId]].actif == true );
		require( hiveParticipants[participantList[_userId]].balance > meetingList[_id].stake);
		
		//get The meeting trough his id

		meetingList[_id].totalStake += meetingList[_id].stake;
		meetingList[_id].meetingParticipant.push(participantList[_userId]);
	}
